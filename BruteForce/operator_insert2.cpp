...
pair<int, int> calc(vector<int> &a, int index, int cur, int plus, int minus, int mul, int div){
    int n = a.size();
    if(index == n){
        return make_pair(cur, cur); // first one used for max, the other for min
    }
    ...
    for(auto p : res){
        if(ans.first < p.first){
            ans.first = p.first; // get the maximum result
        }
        if(ans.second > p.second){
            ans.second = p.second; // get the minimum result
        }
    }
    return ans; // return the pair of max and min
}
...
int main(){
    ...
    int plus, minus, mul, div; // the number of each operator
